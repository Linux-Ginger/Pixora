...
class InstallerWindow(Adw.ApplicationWindow):
    ...
    def _create_launcher(self):
        try:
            ...
            launcher.write_text(
                "#!/bin/bash\n"
                "# Run under AppArmor-profile 'pixora' if available so WebKit's\n"
                "# bwrap-sandbox works on Ubuntu 24.04+. Else plain — fine on\n"
                "# systems without the AppArmor userns restriction.\n"
                f"MAIN={INSTALL_DIR}/viewer/main.py\n"
                ...
                "run_pixora \"$@\"\n"
                "rc=$?\n"
                "# Startup crash while a custom GSK_RENDERER was active: the\n"
                "# sentinel persists only when Pixora died before clearing it.\n"
                "# Relaunch once; next run auto-reverts the renderer to 'auto'.\n"
